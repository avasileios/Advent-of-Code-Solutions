...
def run_script(program, script, mode):
    vm = Intcode(program, [])
    feed = ''.join(s + '\n' for s in script + [mode])
...
def main():
    ...
    # jump if there's a hole at A or (hole at C and ground at D)
    # jump if any of A/B/C is a hole, landing (D) is ground
    script = [
        "OR C J",
        "AND A J",
        "AND B J",
    ...
    print(run_script(program, script, "WALK"))
