...
def run_vm(program, initial_registers=None):
    ...
    registers = initial_registers if initial_registers is not None else {'a': 0, 'b': 0, 'c': 0, 'd': 0}
...
def solve_assembunny_puzzle(filepath):
    """
    Loads the program and runs the simulation starting with a=0, b=0, c=0, d=0.
    """
    ...
    # Run simulation with initial state a=0, b=0, c=0, d=0
    final_registers = run_vm(program)
...
if __name__ == "__main__":
    ...
    print(f"Starting Assembunny simulation using data from: {os.path.abspath(input_file)}\n")
    
    final_a_value = solve_assembunny_puzzle(input_file)
