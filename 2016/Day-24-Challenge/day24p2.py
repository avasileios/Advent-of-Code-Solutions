...
def solve_duct_puzzle_part_2(filepath):
    ...
    for path_order in permutations(destinations):
        ...
        # --- CRITICAL CHANGE: Route must return to start_node (0) ---
        current_route = [start_node] + list(path_order) + [start_node]
        current_cost = 0
...
if __name__ == "__main__":
    ...
    print(f"Starting HVAC duct search (Part 2 - Return to 0) using map from: {os.path.abspath(input_file)}\n")
    
    final_score = solve_duct_puzzle_part_2(input_file)
    
    print("\n" + "="*50)
    print("PART TWO: FEWEST STEPS REQUIRED TO VISIT ALL AND RETURN TO 0:")
