...
# Item Types: 0=Promethium, 1=Cobalt, 2=Curium, 3=Ruthenium, 4=Plutonium, 5=Elerium, 6=Dilithium

def get_hardcoded_initial_state():
    """
    Returns the initial state based on the user's hardcoded input, including 
    the four new items (Elerium and Dilithium pairs) on the first floor.
    """
    
    # 0: Promethium, 1: Cobalt, 2: Curium, 3: Ruthenium, 4: Plutonium, 5: Elerium, 6: Dilithium
    
    # Floor 0 (F1): Initial items + NEW Elerium and Dilithium pairs
    floor0 = frozenset([
        (0, 'G'), (0, 'M'),  # Promethium Pair
        (5, 'G'), (5, 'M'),  # Elerium Pair (NEW)
        (6, 'G'), (6, 'M')   # Dilithium Pair (NEW)
    ])
    
    # Floor 1 (F2): CoG, CmG, RuG, PuG
    floor1 = frozenset([
        (1, 'G'), (2, 'G'), (3, 'G'), (4, 'G')
    ])
    
    # Floor 2 (F3): CoM, CmM, RuM, PuM
    floor2 = frozenset([
        (1, 'M'), (2, 'M'), (3, 'M'), (4, 'M')
    ])
    ...
    item_types = ["Promethium", "Cobalt", "Curium", "Ruthenium", "Plutonium", "Elerium", "Dilithium"]
...
def is_safe(floor_items: Set[Tuple[int, str]]) -> bool:
    ...
    for chip_idx in present_microchips:
        # A chip is fried if it is present AND its corresponding generator is MISSING, 
        # AND at least one other generator is present.
...
def get_all_next_states(current_state: Tuple[int, Tuple], ROWS: int, item_types: List[str], canonical_to_raw_map: Dict) -> List[Tuple[Tuple, Tuple]]:
    ...
    for direction in possible_moves:
        ...
        if direction == -1:
             # Check if all floors below the destination floor are empty. 
             # next_floor is the destination floor index.
             is_any_item_below = any(len(current_floors_raw[f]) > 0 for f in range(next_floor))
             
             # Also check if the destination floor is empty (useless if we move empty to empty).
             is_destination_empty = not current_floors_raw[next_floor]
             
             # If nothing is below the destination AND the destination is empty, skip this downward move.
        ...
        for load in items_to_move:
            ...
            # CRITICAL: Sort the list of (G_floor, M_floor) tuples to ensure symmetry 
            # (swapping Hydrogen for Lithium does not change the canonical state).
...
if __name__ == "__main__":
    ...
    print(f"Starting Radioisotope Facility BFS search (Target F{GOAL_FLOOR + 1}) with hardcoded 7-pair input (Part Two).\n")
