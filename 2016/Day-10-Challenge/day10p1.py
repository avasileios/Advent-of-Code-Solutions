...
def solve_bot_comparison_puzzle(filepath):
    """
    Simulates the robot factory process to find the bot responsible for comparing 
    ...
    # State containers
    # bot_chips: {bot_id: [chip1, chip2, ...]}
    bot_chips = defaultdict(list)
    # outputs: {output_id: [chip1, chip2, ...]}
    ...
    while True:
        ...
        if not ready_bots:
            # If no bots are ready and the process hasn't finished, the system is stable/halted.
            break
        ...
        for bot_id in ready_bots:
            ...
            if low_value == TARGET_LOW and high_value == TARGET_HIGH:
                ...
                # Optimization: Since we found the answer, we can stop the entire process
                return winning_bot_id 
                
            # --- Execute Instruction ---
            if bot_id not in bot_rules:
                # Should not happen with valid input
                bot_chips[bot_id] = [] # Clear chips to prevent infinite loop
                continue
...
if __name__ == "__main__":
    ...
    print(f"Starting robot factory simulation for comparison {TARGET_LOW} vs {TARGET_HIGH} using data from: {os.path.abspath(input_file)}\n")
    
    final_bot_id = solve_bot_comparison_puzzle(input_file)
    
    print("\n" + "="*50)
    print(f"BOT RESPONSIBLE FOR COMPARING VALUE-{TARGET_HIGH} WITH VALUE-{TARGET_LOW}:")
    print(f"SCORE: {final_bot_id}")
    print("="*50)