...
def solve_assembly_puzzle(filepath):
    """
    Loads the program and runs the simulation starting with a=1, b=0 (Part Two).
    """
    ...
    # --- PART TWO: Initial state a=1, b=0 ---
    final_registers = run_vm(program, initial_a=1, initial_b=0)
...
if __name__ == "__main__":
    ...
    print(f"Starting assembly simulation (Part Two: A=1) using data from: {os.path.abspath(input_file)}\n")
    
    final_b_value = solve_assembly_puzzle(input_file)
    
    print("\n" + "="*50)
    print("PART TWO: FINAL VALUE IN REGISTER 'b' (Starting A=1):")
